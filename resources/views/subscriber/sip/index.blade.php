...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('subscriber.sip.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <div class="flex items-center">
                            ...
                            <span class="ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $sip->status === 'active' ? 'green' : $sip->status === 'cancelled' ? 'red' : 'blue' }}-100 text-{{ $sip->status === 'active' ? 'green' : $sip->status === 'cancelled' ? 'red' : 'blue' }}-800">
                                {{ $sip->status }}
                            </span>
                        </div>
                        <p class="text-sm text-gray-500 mt-1">
                            ₹{{ number_format($sip->amount) }} / {{ ucfirst($sip->frequency) }} for {{ $sip->duration }} {{ $sip->frequency === 'weekly' ? 'weeks' : 'months' }}
                        </p>
                        <p class="text-sm text-gray-500 mt-1">
                    ...
                    <div class="flex items-center space-x-3 ml-6">
                        <a href="{{ route('subscriber.sip.show', $sip) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                            View Details
                        </a>
