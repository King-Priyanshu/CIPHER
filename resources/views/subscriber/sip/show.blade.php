...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <!-- SIP Status Badge -->
    <div class="mb-6">
        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-{{ $sip->status === 'active' ? 'green' : $sip->status === 'cancelled' ? 'red' : 'blue' }}-100 text-{{ $sip->status === 'active' ? 'green' : $sip->status === 'cancelled' ? 'red' : 'blue' }}-800">
            {{ ucfirst($sip->status) }}
        </span>
    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $sip->duration }} {{ $sip->frequency === 'weekly' ? 'weeks' : 'months' }}</span>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $sip->start_date->addMonths($sip->duration)->format('M d, Y') }}</span>
                </div>
                <div class="flex justify-between">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium text-gray-900">₹{{ number_format($sip->completed_payments_count * $sip->amount) }}</span>
                </div>
                <div class="flex justify-between">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $payment->status === 'paid' ? 'green' : $payment->status === 'failed' ? 'red' : 'yellow' }}-100 text-{{ $payment->status === 'paid' ? 'green' : $payment->status === 'failed' ? 'red' : 'yellow' }}-800">
                                {{ $payment->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            <a href="{{ route('subscriber.payments.show', $payment->payment) }}" class="text-blue-600 hover:text-blue-900">
                                View Receipt
                            </a>
                            @else
                            -
    ...
    <div class="mt-8 flex items-center space-x-4">
        ...
        <a href="{{ route('subscriber.sip.edit', $sip) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
            Edit SIP
        </a>
        ...
        @endif
        <a href="{{ route('subscriber.sip.index') }}" class="inline-flex items-center px-4 py-2 text-gray-600 text-sm font-medium hover:text-gray-900 transition-colors">
            Back to SIPs
        </a>
    </div>
</div>
