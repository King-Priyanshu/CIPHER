        ...
        <div class="card">
            ...
            <form action="{{ route('admin.investment-plans.update', $investmentPlan) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="project_id" id="project_id" required class="input-field">
                            ...
                            <option value="{{ $project->id }}" {{ $investmentPlan->project_id == $project->id ? 'selected' : '' }}>{{ $project->name ?? $project->title }}</option>
                            @endforeach
                        </select>
                        @error('project_id')
                            ...
                            <div class="mt-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <button type="button" class="text-red-500 hover:text-red-700 delete-benefit">
                ...
                <div class="mt-8 flex justify-end space-x-3 border-t border-gray-100 pt-6">
                    <a href="{{ route('admin.investment-plans.index') }}" class="btn-ghost">Cancel</a>
...
document.addEventListener('DOMContentLoaded', function() {
    // Re-using logic from create.blade.php but adapted for existing items
    const tiersContainer = document.getElementById('tiers-container');
    const addTierButton = document.getElementById('add-tier');
    let tierIndex = {{ count($investmentPlan->tiers ?? []) }};
    
    // Function to add benefit to a container
    function addBenefitListener(triggerButton) {
        triggerButton.addEventListener('click', function() {
            const tierItem = this.closest('.tier-item');
            const benefitsContainer = tierItem.querySelector('.space-y-2');
            
            // Recalculate tier index from DOM position to be safe
            const currentTierIndex = Array.from(tiersContainer.children).indexOf(tierItem);
            ...
            newBenefit.innerHTML = `
                <input type="text" name="tiers[${currentTierIndex}][benefits][${benefitsIndex}]" value=""
                       class="input-field flex-1 mr-2">
                <button type="button" class="text-red-500 hover:text-red-700 delete-benefit">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
            ...
            // Add delete event
            newBenefit.querySelector('.delete-benefit').addEventListener('click', function() {
                this.closest('.flex.items-center').remove();
            });
        });
    }

    // Initialize existing add-benefit buttons
    document.querySelectorAll('.add-benefit').forEach(addBenefitListener);
    
    // Initialize existing delete-benefit buttons
    ...
    addTierButton.addEventListener('click', function() {
        // Find next index
        tierIndex = tiersContainer.children.length;
        ...
        // Add event listener to new benefit button
        const newBtn = newTier.querySelector('.add-benefit');
        addBenefitListener(newBtn);
    });
});
</script>
@endpush
