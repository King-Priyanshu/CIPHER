<x-layouts.admin>
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-white">Create Content Page</h2>
    </div>
        ...
        <form action="{{ route('admin.pages.store') }}" method="POST">
            ...
            <div class="mt-6">
                ...
                <textarea name="content" id="content" rows="10" required
                    class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent font-mono text-sm">{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Create Page</button>
            </div>
        </form>
    </div>
</x-layouts.admin>
